...
export const merge = (a: number[], b: number[]) => {
    ...
    const answer = Array.from({ length: a.length + b.length - 1 }, () => 0)
    let i = 0,
        ai = 0,
        bi = 0
...
for (let i = 1; i < 6; i++) {
    cpList.next = new ListNode(i)
    cpList = cpList.next
}
list = list.next!

export const reverseBetween = function (head: ListNode, left: number, right: number) {
